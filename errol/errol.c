...
inline static void hp_normalize(struct hp_t *hp);
inline static void hp_mul10(struct hp_t *hp);
inline static void hp_div10(struct hp_t *hp);
static struct hp_t hp_prod(struct hp_t in, double val);
inline static int mismatch10(uint64_t a, uint64_t b);
inline static int table_lower_bound(uint64_t *table, int n, uint64_t k);
...
int json_c_errol2_dtoa(double val, char *buf, bool *opt)
{
	if((val <= 9.007199254740992e15) || (val >= 3.40282366920938e38))
		return json_c_errol1_dtoa(val, buf, opt);

	int8_t i, j;
	int exp;
	json_c_errol_bits_t bits;
...
{
	json_c_errol_bits_t k = { val };

	int n = sizeof(json_c_errol_enum3) / sizeof(uint64_t);
	int i = table_lower_bound(json_c_errol_enum3, n, k.i);
	if (i < n && json_c_errol_enum3[i] == k.i)
	{
		strcpy(buf, json_c_errol_enum3_data[i].str);
		return json_c_errol_enum3_data[i].exp;
	}

	return json_c_errol3u_dtoa(val, buf);
}
...
int json_c_errol3u_dtoa(double val, char *buf)
{
	...
	if((val > 9.007199254740992e15) && (val < 3.40282366920938e+38))
		return json_c_errol_int(val, buf);
	else if((val >= 16.0) && (val <= 9.007199254740992e15))
		return json_c_errol_fixed(val, buf);
...
int json_c_errol4_dtoa(double val, char *buf)
{
	...
	int n = sizeof(json_c_errol_enum4) / sizeof(uint64_t);
	int i = table_lower_bound(json_c_errol_enum4, n, k.i);
	if (i < n && json_c_errol_enum4[i] == k.i)
	{
		strcpy(buf, json_c_errol_enum4_data[i].str);
		return json_c_errol_enum4_data[i].exp;
	}

	return json_c_errol4u_dtoa(val, buf);
}
